...
class TheBible_Plugin {
    ...
    private static function inject_nav_helpers($html, $highlight_ids = [], $chapter_scroll_id = null, $book_label = '') {
        ...
        function renderSelectionControls(info){ if(!controls) return; var ref=buildRef(info); var link=buildLink(info); var md="["+ref+"](\n"+link+")"; controls.innerHTML = "<a href=\"#\" class=\"thebible-btn\" data-copy-link>copy link</a> <a href=\"#\" class=\"thebible-btn\" data-copy-text>copy text</a>"; var btnL=controls.querySelector("[data-copy-link]"); var btnT=controls.querySelector("[data-copy-text]"); if(btnL){ btnL.addEventListener("click", function(e){ e.preventDefault(); copyToClipboard(md).then(function(){ btnL.textContent="copied"; setTimeout(function(){btnL.textContent="copy link";},1000); }); }); } if(btnT){ btnT.addEventListener("click", function(e){ e.preventDefault(); var txt=verseText(info); copyToClipboard(txt).then(function(){ btnT.textContent="copied"; setTimeout(function(){btnT.textContent="copy text";},1000); }); }); } }
